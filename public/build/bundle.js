...
var app = (function () {
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(button, file$1, 31, 0, 806);
    		},
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	let counter = writable({ totalSeconds: maxSeconds });
    		...
    		if (intervalHandle) {
    			...
    			intervalHandle = setInterval(
    				() => {
    					...
    					counter.update(value => ({ totalSeconds: value.totalSeconds - 1 }));
    				},
    				1000
    			);

    			$$invalidate(0, status = "Running");
    		}
    	};
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*$counter*/ 32) {
    			$$invalidate(1, seconds = Math.floor($counter.totalSeconds % 60).toString().padStart(2, "0"));
    		}

    		if ($$self.$$.dirty & /*$counter*/ 32) {
    			$$invalidate(2, minutes = Math.floor($counter.totalSeconds / 60).toString().padStart(2, "0"));
    		}
    	};

    	return [status, seconds, minutes, counter, onClick, $counter];
    }
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(h1, file, 6, 1, 83);
    			attr_dev(a, "href", "https://svelte.dev/tutorial");
    			add_location(a, file, 7, 14, 120);
    			add_location(p, file, 7, 1, 107);
    			attr_dev(main, "class", "svelte-1tky8bj");
    			add_location(main, file, 5, 0, 75);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
